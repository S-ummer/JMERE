...
class PromptTrainer:
    def __init__(self, model, config, train_loader, valid_loader, test_loader) -> None:
        ...
        self.final_score = 0
        self.final_res = ''
        self.model_path = '{0}/savemodel8/{1}'.format(self.config.data_dir,self.config.reasoning)
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.config.epoch_size)):
            ...
            self.train_step()
            pre, recall, f1, mac_pre, mac_recall, mac_f1 = self.evaluate_step(dataLoader=self.test_loader)
            score = f1
    ...
    def evaluate_step(self, dataLoader=None, mode='valid'):
        ...
        if dataLoader == self.valid_loader:
            ground_truth_path = os.path.join(self.config.data_dir, "val_ofa_knowledge_image.json")

        else:
            ground_truth_path = os.path.join(self.config.data_dir, "test_ofa_knowledge_image.json")
