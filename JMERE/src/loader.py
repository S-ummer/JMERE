...
class MyDataLoader:
    ...
    def get_prompt(self, prompt_type, line,image_caption,knowledge):
        if prompt_type == "imagecap_know":
            ...
            final_line = "<Img> image caption is: " + image_caption+" <\Img>"+ ". <Text> Text is: " + line+ "<\Text>" +"<Knowledge> the background knowledge: " + knowledge+" <\Knowledge>" \
                        + "."
            ...
            # for one_know in knowledge:
            #     knowledge_str += one_know
            final_line = "<Knowledge> the background knowledge: " + knowledge+" <\Knowledge>"+ "<Text> Text is: " + line+" ."
    ...
    def collate_fn(self, data):
        # print(*data)
        input_tokens, image_caption, knowledge, relation_list, label_list, image_feature = zip(*data)
        ...
        if self.config.reasoning == 'prompt':
            ...
            for i, line in enumerate(input_tokens):
                ...
                batch_image.append(image_feature[i])
            ...
            batch_image = torch.stack(batch_image).to(self.config.device)
...
class Preprocessor:
    ...
    def read_file(self):
        ...
        train_file = os.path.join(self.config.data_dir, "train_ofa_knowledge_image.json")
        valid_file = os.path.join(self.config.data_dir, "val_ofa_knowledge_image.json")
        test_file =  os.path.join(self.config.data_dir, "test_ofa_knowledge_image.json")

        # train_file = os.path.join(self.config.data_dir, "train_ofa_image_caption_knowledge.json")
        # valid_file = os.path.join(self.config.data_dir, "val_ofa_image_caption_knowledge.json")
        # test_file =  os.path.join(self.config.data_dir, "test_ofa_image_caption_knowledge.json")
    ...
    def transformer2indices(self, cur_data):
        ...
        for i,line in enumerate(cur_data):
            ...
            image_feature = line["image_feature"]
            ...
            res.append([text,image_caption,knowledge,relation_list,label_list,image_feature])
        return res
